...
"""
基于图卷积神经网络（Graph Convolutional Neural Network, GCNN）的回归模型，预测波士顿房价数据集中的房价
...
X_train, X_test, y_train, y_test = np.array(
    train_test_split(data_x, data_y, test_size=0.2, random_state=364364)
)
...
def correlation(num_neighbors):
    corr_mat = np.array(
        normalize(np.abs(np.corrcoef(X_train.transpose())), norm="l1", axis=1),
        dtype="float64",
    )
...
def gaussiankernel(num_neighbors, sigma):
    ...
    for i in range(row):
        for j in range(row):
            kernel_mat[i, j] = math.exp(
                -(np.linalg.norm(X_trainT[i] - X_trainT[j]) ** 2) / (2 * sigma**2)
            )
...
def DBSCAN(num_neighbors, num_q):
    ...
    for j in range(X_trainT.shape[0]):
        ...
        for i in node_index_has_target_data:
            ...
            transition_probability = similarity / np.sum(similarity)
            transition_matrix[
                target_index, data_index_in_node
...
model.add(
    GraphConv(
        filters=filters_2, neighbors_ix_mat=graph_mat, num_neighbors=num_neighbors
    )
)
model.add(Activation("relu"))
model.add(
    GraphConv(
        filters=filters_3, neighbors_ix_mat=graph_mat, num_neighbors=num_neighbors
    )
)
model.add(BatchNormalization())
model.add(Activation("relu"))
model.add(
    GraphConv(
        filters=filters_4, neighbors_ix_mat=graph_mat, num_neighbors=num_neighbors
    )
)
model.add(Activation("relu"))
model.add(
    GraphConv(
        filters=filters_5, neighbors_ix_mat=graph_mat, num_neighbors=num_neighbors
    )
)
model.add(Activation("relu"))
...
for i in epochs:
    ...
    pred_train = model.predict(
        X_train.reshape(X_train.shape[0], X_train.shape[1], 1), batch_size=5
    ).flatten()
    pred_test = model.predict(
        X_test.reshape(X_test.shape[0], X_test.shape[1], 1), batch_size=5
    ).flatten()
...
plt.plot(
    epochs[num2 : epoch - num2,],
    smooth1[num2 : epoch - num2,],
    color="cornflowerblue",
    linewidth=3,
    label="Train smooth",
)
