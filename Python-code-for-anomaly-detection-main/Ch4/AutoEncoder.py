...
X_train, X_test = np.array(X_train).astype(np.float32), np.array(X_test).astype(
    np.float32
)
...
class EncDecAD(rm.Model):
    ...
    def forward(self, input_seq, train=False):
        ...
        reconst_seq = np.transpose(
            reconst_seq, (1, 0, 2)
        )  # (time_index, batch, value) => (batch, time_index, value)
...
th = 5256.8384
axes[2].plot(
    [0, 600], [th, th], color="black", linestyle="-", label="threshold", linewidth=1
)
...
axes[2].plot(
    [3000, 5000], [th, th], color="black", linestyle="-", label="threshold", linewidth=1
)
