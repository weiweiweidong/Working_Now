...
class dcgan(rm.Model):
    ...
    def forward(self, x):
        ...
        self.dis_loss_real = rm.sigmoid_cross_entropy(
            self.real_dis, np.ones(batch).reshape(-1, 1)
        )
        self.dis_loss_fake = rm.sigmoid_cross_entropy(
            self.fake_dis, np.zeros(batch).reshape(-1, 1)
        )
        self.dis_loss = self.dis_loss_real + self.dis_loss_fake
        if self.minimax:
            self.gen_loss = -self.dis_loss
        else:  # non-saturating
            self.gen_loss = rm.sigmoid_cross_entropy(
                self.fake_dis, np.ones(batch).reshape(-1, 1)
            )
...
for epoch in trange(1, gan_epoch + 1):
    ...
    if epoch % 10 == 0:
        print(
            "Epoch {}  Loss of Dis {:.3f} Loss of Gen {:.3f} Accuracy of Real {:.3f} Accuracy of Fake {:.3f}".format(
                epoch,
                loss_curve_dis[-1],
                loss_curve_gen[-1],
                acc_curve_real[-1],
...
nrows = 4
z = (
    np.random.randn(ncols * nrows * dim_z)
    .reshape((ncols * nrows, dim_z))
    .astype(np.float32)
)
